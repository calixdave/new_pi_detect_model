...
def detect_color_slots(img, model, classes, debug_prefix=None):
    roi, roi_y0, _ = get_bottom_roi(img)
    slots, boxes = split_into_3_slots(roi, SLOT_PAD_X_FRAC, SLOT_PAD_Y_FRAC)
    ...
    for idx, slot in enumerate(slots):
        ...
        if SAVE_DEBUG and debug_prefix is not None:
            ...
            cv2.imwrite(os.path.join(DEBUG_DIR, f"{debug_prefix}_color_slot{idx}.jpg"), slot)
...
def detect_object_slots(img, debug_prefix=None):
    roi, roi_y0, _ = get_bottom_roi(img)
    slots, boxes = split_into_3_slots(roi, OBJ_PAD_X_FRAC, OBJ_PAD_Y_FRAC)
    ...
    for idx, slot in enumerate(slots):
        ...
        if SAVE_DEBUG and debug_prefix is not None:
            os.makedirs(DEBUG_DIR, exist_ok=True)
            cv2.imwrite(os.path.join(DEBUG_DIR, f"{debug_prefix}_obj_slot{idx}.jpg"), slot)
...
def main():
    ...
    # optional save
    with open("voted_local_color.txt", "w") as f:
        f.write(pretty_matrix(final_color))

    with open("voted_local_object.txt", "w") as f:
        f.write(pretty_matrix(final_object))

    print("\nSaved:")
    print("  voted_local_color.txt")
    print("  voted_local_object.txt")
